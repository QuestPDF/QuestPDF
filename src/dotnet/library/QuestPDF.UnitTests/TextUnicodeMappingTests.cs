...
public class TextUnicodeMappingTests
{
    ...
    {
        var maps = ExtractToUnicodeCmaps(RenderDocument());
    ...
    private static ICollection<string> ExtractToUnicodeCmaps(byte[] document)
    {
        using var pdf = PdfInspector.Load(document);
