...
internal static class TemplateManager
{
    ...
    static TemplateManager()
    {
        ...
        Options.Filters.AddFilter("camelCase", (input, _, _) => new StringValue(input.ToStringValue().ToCamelCase()));
        Options.Filters.AddFilter("pascalCase", (input, _, _) => new StringValue(input.ToStringValue().ToPascalCase()));
        Options.Filters.AddFilter("screamingSnakeCase", (input, _, _) => new StringValue(input.ToStringValue().ToSnakeCase().ToUpperInvariant()));

        Parser = new FluidParser(new FluidParserOptions { AllowFunctions = true });
    }
